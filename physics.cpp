...
void calcul_pos(const physics_constants& physics,int nbvaisseau,struct vaisseau_data *vaisseau,struct platform_data platforms[],int nbplatforms)
{
	while(nbvaisseau--)
	{
      ...
      if (!vaisseau->landed)
      {
      vaisseau->ax = fixmul(vaisseau->thrust, fixsin(itofix(vaisseau->angle)));            // ax = thrust * sin�
      vaisseau->ay = fixadd(physics.iG, fixmul(vaisseau->thrust, -fixcos(itofix(vaisseau->angle))));// ay = g + thrust * (-cos�)
	  // gestion impact
