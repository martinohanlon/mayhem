...
{
public:
	//static void init() { reset(); install_timer(); install_int(&InterruptTimer::irq, 10); };	// install interrupt timer 1/40s (25 ms)
    static void init() { reset(); install_timer(); LOCK_VARIABLE(timing_counter); LOCK_FUNCTION(&InterruptTimer::irq); install_int_ex(&InterruptTimer::irq, BPS_TO_TIMER(40)); };	// install interrupt timer 1/40s (25 ms)
